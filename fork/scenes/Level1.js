class Level1 extends Phaser.Scene { // eslint-disable-line no-undef no-unused-vars
  constructor () {
    super({
      key: 'Level1'
    })
  ...
  create () {
    ...
    this.badguy = this.add.group({
      ...
      setXY: {
        ...
        // Sets monster speed
    Phaser.Actions.Call(this.badguy.getChildren(), function (enemy) { // eslint-disable-line no-undef
      enemy.speed = Math.random() * 2 + 1
    }, this)
        // Scale the monster
    Phaser.Actions.ScaleXY(this.badguy.getChildren(), -0.5, -0.5) // eslint-disable-line no-undef
  ...
  update () {
    ...
    for (let i = 0; i < numEnemies; i++) {
      ...
      if (Phaser.Geom.Intersects.RectangleToRectangle(this.player.getBounds(), enemies[i].getBounds())) { // eslint-disable-line no-undef
        this.gameOver()
        break
      }
    }
